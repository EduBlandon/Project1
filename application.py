...
def review(id):
    ...
    if request.method == "POST":
        ...
        db.execute(
            "INSERT INTO reseñas (book_id, comentario, puntuacion, user_id) VALUES (:id, :comentario, :puntuacion, :user_id)", {
    ...
    reviews = db.execute(
        "SELECT * FROM reseñas as r inner join users as u on r.user_id = u.id  WHERE book_id = :id", {"id": id})

    user_id = session["user_id"]

    count_comment = db.execute(
        "SELECT count(*)as conteo from reseñas where user_id = :user_id and book_id = :id", {"user_id": user_id, "id": id}).fetchone()["conteo"]
...
def get_book_by_code(code):
    ...
    review_count = db.execute(
        "SELECT count(*)as conteo from reseñas where book_id = :code", {"code": book["id"]}).fetchone()["conteo"]
